...
final class LeagueController extends AbstractController
{
  ...
  /*  #[Route('/{id}', name: 'app_league_show', methods: ['GET'])]
    ...
    #[Route('/{id}', name: 'app_league_show', methods: ['GET'])]
public function show(League $league, FixtureRepository $fixtureRepository): Response
{
    // Fetch all fixtures for this league
    $fixtures = $fixtureRepository->findBy(['league' => $league]);

    return $this->render('league/show.html.twig', [
        'league' => $league,
        'fixtures' => $fixtures, // pass fixtures to Twig
    ]);
}
